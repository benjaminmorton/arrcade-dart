...
class _State extends State<ConfigurationGeneralRoute>
  ...
  Widget _openDrawerOnBackAction() {
    const _db = ArrcadeDatabase.ANDROID_BACK_OPENS_DRAWER;
  ...
  Widget _amoledTheme() {
    const _db = ArrcadeDatabase.THEME_AMOLED;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
  ...
  Widget _amoledThemeBorders() {
    return LunaBox.arrcade.listenableBuilder(
      ...
      builder: (context, _) => LunaBlock(
        ...
        trailing: LunaSwitch(
          value: ArrcadeDatabase.THEME_AMOLED_BORDER.read(),
          onChanged: ArrcadeDatabase.THEME_AMOLED.read()
              ? ArrcadeDatabase.THEME_AMOLED_BORDER.update
              : null,
        ),
      ),
    );
  ...
  Widget _imageBackgroundOpacity() {
    const _db = ArrcadeDatabase.THEME_IMAGE_BACKGROUND_OPACITY;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
        title: 'settings.BackgroundImageOpacity'.tr(),
        body: [
          TextSpan(
            text: _db.read() == 0 ? 'arrcade.Disabled'.tr() : '${_db.read()}%',
          ),
        ],
  ...
  Widget _useTLSValidation() {
    const _db = ArrcadeDatabase.NETWORKING_TLS_VALIDATION;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
  ...
  Widget _use24HourTime() {
    const _db = ArrcadeDatabase.USE_24_HOUR_TIME;
    return _db.listenableBuilder(
      builder: (context, _) => LunaBlock(
