...
class _State extends State<ConfigurationDrawerRoute>
    ...
    return Column(
      children: [
        ...
        LunaBlock(
          ...
          trailing: ArrcadeDatabase.DRAWER_AUTOMATIC_MANAGE.listenableBuilder(
            builder: (context, _) => LunaSwitch(
              value: ArrcadeDatabase.DRAWER_AUTOMATIC_MANAGE.read(),
              onChanged: ArrcadeDatabase.DRAWER_AUTOMATIC_MANAGE.update,
            ),
          ),
        ),
        LunaDivider(),
        Expanded(
          child: LunaReorderableListViewBuilder(
            ...
            onReorder: (oIndex, nIndex) {
              ...
              ArrcadeDatabase.DRAWER_MANUAL_ORDER.update(_modules!);
            },
          ),
        ),
      ],
  ...
  Widget _reorderableModuleTile(int index) {
    return ArrcadeDatabase.DRAWER_AUTOMATIC_MANAGE.listenableBuilder(
      key: ObjectKey(_modules![index]),
      builder: (context, _) => LunaBlock(
        disabled: ArrcadeDatabase.DRAWER_AUTOMATIC_MANAGE.read(),
        title: _modules![index].title,
        body: [TextSpan(text: _modules![index].description)],
        leading: LunaIconButton(icon: _modules![index].icon),
        trailing: ArrcadeDatabase.DRAWER_AUTOMATIC_MANAGE.read()
            ? null
