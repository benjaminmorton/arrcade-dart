...
import 'package:arrcade/extensions/duration/timestamp.dart';
...
extension TautulliSessionAudioExtension on TautulliSession {
  String get _language {
    if (audioLanguage?.isEmpty ?? true) return 'arrcade.Unknown'.tr();
...
extension TautulliSessionContainerExtension on TautulliSession {
  String get _container {
    if (container?.isEmpty ?? true) return 'arrcade.Unknown'.tr();
    return container!.toUpperCase();
  }

  String get _streamContainer {
    if (streamContainer?.isEmpty ?? true) return 'arrcade.Unknown'.tr();
...
extension TautulliSessionSubtitleExtension on TautulliSession {
  String get _language {
    if (subtitleLanguage?.isEmpty ?? true) return 'arrcade.Unknown'.tr();
...
extension TautulliSessionVideoExtension on TautulliSession {
  String get _codec {
    if (videoCodec?.isEmpty ?? true) return 'arrcade.Unknown'.tr();
    return videoCodec!.toUpperCase();
  }

  String get _streamCodec {
    if (streamVideoCodec?.isEmpty ?? true) return 'arrcade.Unknown'.tr();
    return streamVideoCodec!.toUpperCase();
  }

  String get _fullResolution {
    if (videoFullResolution?.isEmpty ?? true) return 'arrcade.Unknown'.tr();
