...
import 'package:arrcade/types/exception.dart';
...
class LunaProfileTools {
  bool changeTo(
    ...
    try {
      if (ArrcadeDatabase.ENABLED_PROFILE.read() == profile) return true;
      _changeTo(profile);
  ...
  void _changeTo(String profile) {
    ...
    ArrcadeDatabase.ENABLED_PROFILE.update(profile);
    LunaState.reset();
  }
  ...
  Future<void> _remove(String profile) async {
    if (ArrcadeDatabase.ENABLED_PROFILE.read() == profile) {
