...
* Copy any module's device-based or user-based webhook URL from Arrcade
...
{% swagger baseUrl="https://notify.arrcade.app" path="/v1/custom/device/:device_id" method="post" summary="Device-Based" %}
{% swagger-description %}
Send a custom notification using a device token to a single device running Arrcade.
...
{% swagger baseUrl="https://notify.arrcade.app" path="/v1/custom/user/:user_id" method="post" summary="User-Based" %}
{% swagger-description %}
Send a custom notification using a user token to all devices signed into that Arrcade account.
{% endswagger-description %}
